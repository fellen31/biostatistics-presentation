...
```{r}
# We chose linear regression (as prescribed by the exam)
```


```{r}
# "When to prefer regression:
#   - If you are interested in the line itself (e.g. the slope), use regression.
...
autoplot(group_week)[2]

augment(group_week) %>% pull(.resid) %>% shapiro.test()
